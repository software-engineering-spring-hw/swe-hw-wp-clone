...
export const getUsersWithLatestMessage = (offset: string, limit: string) => {
  ...
  u.last_name as "lastName", u.image,m.content, m.created_at as "createdAt",
  u.user_notes as "userNotes"
  from (select m.*, case when sender_id = ? then recipient_id else sender_id end as other_user_id
